...
v16.0 — BIERMANN BATTERY RE-SEED ON DRIVE RISING-EDGE EVENTS
        Adds physics-accurate re-injection of the Biermann seed field at the
        rising edge of each drive-envelope cycle, addressing the gap diagnosed
        in the parked Paper 2 sweep where modulation only toggled the synthetic
        Jy current and never refreshed the underlying seed.

PHYSICS RATIONALE:
  WarpX hybrid-PIC's isothermal electron pressure closure forces ∇T_e ≡ 0,
  making the Biermann battery term ∇T_e × ∇n_e in the generalized Ohm's law
  vanish identically. The seed field initialized by AnalyticInitialField at
  t=0 therefore never refreshes — modulation of the external Jy current
  toggles imposed field topology, but cannot regenerate gradient-driven
  seeds the way real laser pulses would.

  In a real experiment, each on-cycle of the laser deposits energy locally,
  creating ∇T_e roughly radial about each spot centroid. Combined with the
  pre-existing density Gaussian, the cross product ∇T_e × ∇n_e drives
  ∂By/∂t producing a fresh Biermann bump every cycle. Published scaling
  laws (Bonasera-group, Gao 2016, Santos 2018) give ΔBy ~ 50-300 T per
  pulse at our intensity (~6e13 W/cm² at 5 J / 75 µm).

  v16 reproduces this physics by detecting drive-envelope rising-edge events
  and adding a calibrated ΔBy(x,z) increment to the WarpX By MultiFab at
  each event. The spatial profile reuses the alternating-polarity Harris
  pattern from build_By_expression; the amplitude is per-pulse-calibrated
  (default 100 T, configurable); and local density coupling provides
  natural saturation (only inject where there is dense plasma to be heated).

WHAT'S NEW IN v16.0:
  --biermann-reseed
      Enable per-cycle Biermann seed re-injection. Requires an active drive
      envelope (--pulsed-drive, --wind-and-release, or --schedule-file).
      Default: OFF (v15-compatible behavior).

  --biermann-reseed-amplitude-T VAL
      Peak ΔBy added at each rising-edge event, in Tesla. Default: 100.0.
      Calibrated against published Biermann measurements at comparable
      intensity. Use 0 to disable injection while keeping logging active
      (diagnostic-only mode for testing the callback architecture).

  --biermann-reseed-threshold VAL
      Drive-envelope threshold for rising-edge detection (0.0-1.0).
      Reseed fires when envelope crosses this value going upward.
      Default: 0.5 (catches midpoint of cos² ramp-up cleanly).

  --biermann-reseed-density-couple {none,analytic}
      Spatial coupling of ΔBy to local plasma density.
        none     : flat injection — ΔBy(x,z) = amplitude * seed_profile(x,z)
        analytic : (default) ΔBy *= n_e_analytic(x,z) / n_peak, providing
                   natural saturation as the density profile is recomputed
                   from the analytic ring expression. Note: this does NOT
                   yet track live plasma depletion; future enhancement.

  --biermann-reseed-topology {match-seed,harris,vortex_same,vortex_alt}
      Spatial topology of the injected ΔBy. Default: match-seed (uses the
      same topology as --seed-topology so the t=0 init and per-cycle
      injection are consistent).

DIAGNOSTICS:
  biermann_reseed_events.csv — one row per reseed event with columns:
    step, t_ps, cycle_or_pulse_idx, drive_env_at_trigger, peak_dB_T_injected,
    n_e_max_at_spots, cumulative_reseed_count, write_status

  At startup, a write-access probe verifies whether the pywarpx field-write
  API succeeds for this build. If write access fails, the feature is
  disabled and the run proceeds as v15 (with a clear log message).

USAGE EXAMPLE (frequency sweep near the 28 GHz natural mode with reseed):
  python pb11_ring_reconnection_v16_biermann_reseed.py \\
    --rotate --freq 28e9 --pulsed-drive --pulse-count 280 \\
    --pulse-period-ps 35.7 --pulse-width-ps 17 \\
    --biermann-reseed --biermann-reseed-amplitude-T 100 \\
    --outdir runs/p2v16_reseed_28ghz_dB100

CALIBRATION SWEEP (recommended first run set):
  Sweep --biermann-reseed-amplitude-T over {0, 25, 50, 100, 200} at a
  single modulation frequency. If gain scales monotonically with amplitude,
  the reseed is doing real work. If gain is flat, modulation is genuinely
  null and the v16 mechanism does not change the conclusion.

LIMITATIONS (to be addressed in v17+ if results warrant):
  - Density coupling uses the analytic spot Gaussian, not the live plasma
    density. This does not track plasma depletion between pulses.
  - Spatial topology of injection is fixed (matches the t=0 seed); does
    not yet support spot precession (would be a v17 feature).
  - Injection is impulsive on the rising edge, not distributed over the
    pulse duration. Acceptable because real Biermann saturates within
    a few ps inside a ~17 ps pulse.
...
# PATCHED by apply_v15_fix_mpi_deadlock_in_diag.py on import
# Backup: pb11_ring_reconnection_v15_pulsed.py.bak.before_mpi_deadlock_fix
# Effect: fusion_diagnostic_callback no longer deadlocks on rank-asymmetric
#         particle access.  All ranks participate in every allreduce.



# PATCHED by apply_v15_align_late_diag_to_dump_period.py on import
# Backup: pb11_ring_reconnection_v15_pulsed.py.bak.before_align_late_diag
# Effect: production-profile LATE_DIAG_PERIOD now aligns with --dump-period.
#         Fixes empty fusion_rate_power_by_iter.csv for short UUF runs.



# PATCHED by apply_v15_skip_final_diag_callback.py on import
# Backup: pb11_ring_reconnection_v15_pulsed.py.bak.before_skip_final_diag
# Effect: fusion_diagnostic_callback skips step==n_steps (final step)
#         Prevents MPI deadlock from rank-asymmetric particle access.



# PATCHED by apply_v15_add_amrex_arena_caps.py on import
# Backup: pb11_ring_reconnection_v15_pulsed.py.bak.before_arena_caps
# Effect: AMReX arena init size capped (4 GB host, 8 GB device per rank)
#         Prevents 8-rank OOM under frequent diagnostic dumps.



# PATCHED by apply_v15_add_nppc_substeps_args.py on import
# Backup: pb11_ring_reconnection_v15_pulsed.py.bak.before_add_nppc_substeps
# Effect: added --nppc and --substeps CLI flags; substeps auto-scales
#         with NX (80 at 256², 160 at 512²); NPPC defaults preserved.



# PATCHED by apply_v15_fix_eff_n_target.py on import
# Backup: pb11_ring_reconnection_v15_pulsed.py.bak.before_eff_n_target_fix
# Effect: effective_target_density() is now an area-weighted average,
#         not a sum of zone densities.  Fusion rates in CSV are now
#         spatial averages over the box, not over-counted multiples.
#         First-transit analysis remains the canonical zone-local source.



# PATCHED by apply_v15_fix_accounting_notes.py on import
# Backup: pb11_ring_reconnection_v15_pulsed.py.bak.before_accounting_fix
# Effect: fusion_accounting_notes.txt now reports rod_fuel/ring_fuel=none
#         when the region is disabled, not the stale argparse default.



# PATCHED by apply_v15_drop_centre_zone_cols.py on import
# PATCHED by apply_v15_fix_fuel_any_contamination.py on import
# PATCHED by apply_v15_add_rod_outer_geometry_keys.py on import
# Applied: 2026-05-12T19:24:19
# Effect: rod_radius_m / outer_radius_m / outer_thickness_m / lx_m / lz_m
#         added to run_meta [geometry] block, conditional on FUEL_ROD
#         and FUEL_RING_ANY.  Required by pb11_first_transit_fusion.py
#         and pb11_zone_analysis_paper3.py.
# Applied: 2026-05-12T19:24:19
# Effect: FUEL_ANY (legacy = rod or ring) split from FUEL_RING_ANY
#         (= ring-only).  Fixes run_meta showing ring enabled=True
#         when only --fuel-rod is passed.
# Backup: pb11_ring_reconnection_v15_pulsed.py.bak.before_drop_centre_cols
# Effect: removed 5 centre-zone columns from fusion_rate_power_by_iter.csv
#         (R_CENTRE=0.25*R_RING is geometrically wrong for ring config)
# Authoritative: pb11_zone_analysis_paper3.py Stage D, consolidate_reports Stage 2



import argparse

# v15.3: schedule-file support (composable multi-phase drive schedules).
# Generalises v15.2 wind-and-release into arbitrary cycle counts via CSV.
_VERSION = "15.3"
# ── v15.3: schedule file parsing ─────────────────────────────────────────────
def parse_schedule_file(path):
    """Parse a v15.3 schedule CSV into a list of phase dicts.

    Required CSV columns:
      cycle        (int >= 1)
      phase_type   ('charge' or 'release')
      duration_ps  (float > 0)
      ramp_ps      (float >= 0, must be <= duration_ps)
      j_scale      (float in [0, 1])

    Each phase ramps from the previous phase's target j_scale (or 0 for the
    first phase) to its own target over ramp_ps using cos^2/sin^2
    interpolation, then holds at the target for the remainder of duration_ps.
    After the final phase, the envelope is permanently zero.

    Returns a list of dicts, each with keys:
      cycle, phase_type, duration_s, ramp_s, j_scale, phase_index
    where phase_index is a 0-indexed counter across the whole schedule.

    Raises ValueError on any parse or validation failure.
    """
    import csv as _csv
    valid_phase_types = {'charge', 'release'}
    required = {'cycle', 'phase_type', 'duration_ps', 'ramp_ps', 'j_scale'}

    phases = []
    with open(path, 'r', newline='', encoding='utf-8') as fh:
        reader = _csv.DictReader(fh)
        if reader.fieldnames is None:
            raise ValueError(f'schedule file {path}: empty or missing header')
        header = {name.strip() for name in reader.fieldnames}
        missing = required - header
        if missing:
            raise ValueError(
                f'schedule file {path}: missing required columns {sorted(missing)}. '
                f'Required: {sorted(required)}'
            )
        for row_idx, row in enumerate(reader, start=2):
            try:
                cycle       = int(row['cycle'])
                phase_type  = str(row['phase_type']).strip().lower()
                duration_ps = float(row['duration_ps'])
                ramp_ps     = float(row['ramp_ps'])
                j_scale     = float(row['j_scale'])
            except (KeyError, ValueError, TypeError) as e:
                raise ValueError(f'schedule file {path}, line {row_idx}: parse error - {e}')

            if phase_type == 'hold':
                raise ValueError(
                    f"schedule file {path}, line {row_idx}: phase_type 'hold' "
                    f"not supported in v15.3, planned for future. "
                    f"Use 'charge' or 'release' for now."
                )
            if phase_type not in valid_phase_types:
                raise ValueError(
                    f"schedule file {path}, line {row_idx}: unknown phase_type "
                    f"'{phase_type}'. Must be one of {sorted(valid_phase_types)}."
                )
            if cycle < 1:
                raise ValueError(
                    f'schedule file {path}, line {row_idx}: cycle must be >= 1, got {cycle}'
                )
            if duration_ps <= 0:
                raise ValueError(
                    f'schedule file {path}, line {row_idx}: duration_ps must be > 0, '
                    f'got {duration_ps}'
                )
            if ramp_ps < 0:
                raise ValueError(
                    f'schedule file {path}, line {row_idx}: ramp_ps must be >= 0, '
                    f'got {ramp_ps}'
                )
            if ramp_ps > duration_ps:
                raise ValueError(
                    f'schedule file {path}, line {row_idx}: ramp_ps ({ramp_ps}) > '
                    f'duration_ps ({duration_ps}). Ramp must fit within phase duration.'
                )
            if not (0.0 <= j_scale <= 1.0):
                raise ValueError(
                    f'schedule file {path}, line {row_idx}: j_scale must be in [0, 1], '
                    f'got {j_scale}'
                )
            ...
            phases.append({
                ...
                'ramp_s': ramp_ps * 1e-12,
                'j_scale': j_scale,
                'phase_index': len(phases),
            })

    if not phases:
        raise ValueError(f'schedule file {path}: no phases found (only header?)')
...
parser.add_argument('--nppc',         type=int, default=0,
                    help='Macroparticles per cell; 0=auto by --test (test=200, production=400)')
parser.add_argument('--substeps',     type=int, default=0,
                    help='Hybrid-PIC field solver substeps; 0=auto-scale with NX '
...
parser.add_argument('--rotation-pattern', type=str, default='full',
                    choices=['full', 'two_pair', 'single_pair'],
                    help='Spatial pattern of the rotating drive (v15.3+, only active when '
                         '--rotate is set):\n'
                         '  full        - (default) all N_SPOTS active simultaneously,\n'
                         '                rotating as a unit. Legacy v13/v14/v15 behavior.\n'
                         '  two_pair    - 4 active spots forming two perpendicular diametric\n'
                         '                pairs at angles {0, pi/2, pi, 3pi/2} + 2*pi*f*t,\n'
                         '                polarity (+,-,+,-). 4-fold pattern symmetry.\n'
                         '  single_pair - 2 active spots forming one diametric pair at\n'
                         '                angles {0, pi} + 2*pi*f*t, polarity (+,-).\n'
                         '                2-fold pattern symmetry.\n'
                         'Note: X-line diagnostic positions remain at the 8 nominal static\n'
                         'positions regardless of pattern; the drive sweeps through them.\n'
                         'Polarity selection respects --rotate-mode: in driving mode all\n'
                         'active spots are same-sign; in perturbative mode they alternate.')
...
# v15: PULSED DRIVE — open-loop scheduled pulses with energy budget.
# Tests the hypothesis (vs continuous drive) that pumping energy into the
# field continuously prevents reconnection from completing. By driving in
# pulses synced to the natural reconnection cycle (~28 GHz / 35 ps), the
# drive rebuilds the field after reconnection has consumed it, then steps
# aside to let the next reconnection event accelerate particles.
#
# Implementation: pure analytic envelope multiplier on the rotating Jy
# expression. No callbacks needed. Each pulse is a smooth (cos^2) bump
# centered every PULSE_PERIOD_PS, with FWHM = PULSE_WIDTH_PS. After the
# Nth pulse, envelope is identically zero (drive permanently off).
parser.add_argument('--pulsed-drive', action='store_true',
                    help='Activate pulsed mode: rotating Jy is multiplied by a '
                         'time-dependent pulse envelope. Without this flag, '
                         'the rotating drive is continuous (matches v13/v14).')
parser.add_argument('--pulse-count', type=int, default=77,
                    help='Total pulses to fire before drive permanently turns off. '
                         'Default 77 corresponds to ~5 J injected at j_scale=0.5 '
                         'over a 2.7 ns run with 35 ps period. After N pulses, '
                         'the simulation continues with no drive, allowing post-'
                         'budget particle dynamics to be observed.')
parser.add_argument('--pulse-period-ps', type=float, default=35.0,
                    help='Time between pulse starts (in picoseconds). Default 35.0 '
                         'matches the ~28 GHz natural reconnection cycle estimate. '
                         'One pulse fires per natural cycle.')
parser.add_argument('--pulse-width-ps', type=float, default=17.0,
                    help='Pulse width FWHM in picoseconds. Default 17.0 = half of '
                         'a 35 ps cycle. Drive is on for half the cycle (rebuild '
                         'field), off for the other half (let reconnection complete '
                         'and accelerate particles).')
parser.add_argument('--pulse-shape', type=str, default='cos2',
                    choices=['square', 'cos2', 'gauss'],
                    help='Pulse envelope shape:\n'
                         '  cos2   - (default) cos^2 envelope, smooth on/off, '
                         'spectrally clean. Width = FWHM.\n'
                         '  gauss  - Gaussian envelope, sigma = pulse_width / (2*sqrt(2*ln(2))).\n'
                         '  square - Sharp on/off (introduces broadband forcing, '
                         'use only for diagnostic comparison).')

# v15.2: WIND-AND-RELEASE — single charge phase followed by drive-off observation.
# Tests whether continuous drive followed by abrupt cessation produces a larger
# reconnection event than the natural baseline (i.e., whether the magnetic-pump
# artifact, when intentionally exploited, releases stored energy via reconnection).
#
# Schedule:
#   t = [0, ramp_in_ps]                     : drive ramps up (cos^2)
#   t = [ramp_in_ps, charge_duration_ps]    : drive at full amplitude (charging)
#   t = [charge_duration_ps,
#        charge_duration_ps + ramp_out_ps]  : drive ramps down (cos^2)
#   t > charge_duration_ps + ramp_out_ps    : drive identically zero (release)
#
# Mutually exclusive with --pulsed-drive. Only active when --wind-and-release is
# set; without it, drive behaves per other flags (continuous, pulsed, etc.).
parser.add_argument('--wind-and-release', action='store_true',
                    help='Activate wind-and-release mode: single charge phase '
                         '(continuous drive) followed by drive cessation. Tests '
                         'whether stored magnetic energy can be released as a '
                         'large reconnection event when drive is removed. '
                         'Mutually exclusive with --pulsed-drive.')
parser.add_argument('--charge-duration-ps', type=float, default=300.0,
                    help='Duration (ps) of the charging phase during which drive '
                         'is at full amplitude. After this, drive ramps down and '
                         'turns off for the remainder of the simulation. Default '
                         '300 ps. Larger values pump field higher; shorter values '
                         'preserve more of the natural reconnection topology.')
parser.add_argument('--charge-ramp-in-ps', type=float, default=5.0,
                    help='Cos^2 ramp-up duration at start of charging phase. '
                         'Default 5 ps prevents impulsive turn-on while keeping '
                         'most of charge_duration at full amplitude.')
parser.add_argument('--charge-ramp-out-ps', type=float, default=5.0,
                    help='Cos^2 ramp-down duration at end of charging phase. '
                         'Default 5 ps. Sharp release maximizes dB/dt at cessation; '
                         'longer ramps allow gradual energy release. Set to 0 for '
                         'square turnoff (introduces broadband forcing).')
parser.add_argument('--schedule-file', type=str, default='',
                    help='v15.3: Path to a CSV file defining a multi-phase drive schedule. '
                         'Generalises wind-and-release to arbitrary cycle counts. '
                         'CSV columns: cycle, phase_type, duration_ps, ramp_ps, j_scale. '
                         "phase_type must be 'charge' or 'release' in v15.3. "
                         'Each phase ramps from the previous phase\'s target j_scale to '
                         'its own target over ramp_ps, then holds at target for the rest '
                         'of duration_ps. Mutually exclusive with --pulsed-drive and '
                         '--wind-and-release.')

# ── v16: Biermann battery re-seed on drive rising-edge events ───────────────
# When a drive envelope is active (--pulsed-drive, --wind-and-release, or
# --schedule-file), the rising edge of each cycle triggers an injection of
# ΔBy(x,z) into the WarpX By MultiFab. This addresses the physics gap where
# the isothermal electron closure forbids natural Biermann generation, so the
# t=0 seed never refreshes when the drive is modulated.
parser.add_argument('--biermann-reseed', action='store_true',
                    help='v16: Enable per-cycle Biermann seed re-injection on '
                         'drive rising-edge events. Requires an active drive '
                         'envelope (--pulsed-drive, --wind-and-release, or '
                         '--schedule-file). Default OFF (v15-compatible).')
parser.add_argument('--biermann-reseed-amplitude-T', type=float, default=100.0,
                    help='v16: Peak ΔBy (Tesla) added at each rising-edge '
                         'event. Default 100 T (midpoint of published Biermann '
                         'scaling at ~6e13 W/cm² and 75 µm spot). Set to 0 to '
                         'disable injection while keeping the event log active '
                         '(diagnostic-only mode).')
parser.add_argument('--biermann-reseed-threshold', type=float, default=0.5,
                    help='v16: Drive-envelope threshold (0-1) for rising-edge '
                         'detection. Reseed fires once per cycle when envelope '
                         'first crosses this value upward. Default 0.5.')
parser.add_argument('--biermann-reseed-density-couple', type=str,
                    default='analytic', choices=['none', 'analytic'],
                    help='v16: Spatial coupling of ΔBy to local density.\n'
                         '  none     - ΔBy = amplitude * seed_profile(x,z)\n'
                         '  analytic - (default) multiply by analytic '
                         'n_e(x,z)/n_peak. Provides spot localization but does '
                         'NOT yet track live plasma depletion.')
parser.add_argument('--biermann-reseed-topology', type=str,
                    default='match-seed',
                    choices=['match-seed', 'harris', 'vortex_same', 'vortex_alt'],
                    help='v16: Spatial topology of injected ΔB. Default '
                         '"match-seed" reuses the topology selected by '
                         '--seed-topology. Use other values to test mismatched '
                         'injection topologies as a control.')
parser.add_argument('--biermann-reseed-write-probe-only', action='store_true',
                    help='v16: DIAGNOSTIC mode — run the field-write probe at '
                         'startup, log the result, then exit without stepping '
                         'the simulation. Use to verify whether pywarpx '
                         'field-write API is available for this build before '
...
# v15.3: enforce mutual exclusion across all three drive modes
_n_drive_modes = sum([
    bool(args.wind_and_release),
    bool(args.pulsed_drive),
    bool(args.schedule_file),
])
if _n_drive_modes > 1:
    parser.error('--wind-and-release, --pulsed-drive, and --schedule-file are '
                 'mutually exclusive. Use at most one (or none for continuous drive).')

# v15.3: parse schedule file (if provided) early so banner and build_Jy see it.
_schedule_phases = []
if args.schedule_file:
    _schedule_phases = parse_schedule_file(args.schedule_file)
...
FUEL_RING_ANY   = any([FUEL_RING_INNER, FUEL_RING_OUTER, FUEL_RING_FULL])
FUEL_ANY        = FUEL_ROD or FUEL_RING_ANY  # legacy: any fuel placed (used by species_needed)
...
#
# PATCHED: domain-area-weighted average instead of zone-additive sum.
# The original sum overstated the rate seen by fast protons in multi-zone
# runs (e.g. rod 5e25 + base 5e24 + outer 5e24 gave eff_n = 1.5e25, but most
# fast protons live near the X-line annulus which has only base density 5e24).
# For canonical per-zone accounting, use pb11_first_transit_fusion.py — the
# inline CSV is an upper bound for trend visualization only.
def _zone_areas(sp_key):
    """Approximate cross-sectional areas of each fuel region (m²).

    Uses the 2.4σ-captures-94% Gaussian approximation for the rod, and
    a 2σ annulus for the outer.  Base is everything else."""
    import numpy as _np
    # Read zone radii from globals defined elsewhere in v15.
    # Default to 0 (zone disabled) if not present in scope.
    try:
        _rod_r   = ROD.get('radius_m', 0.0) if FUEL_ROD else 0.0
        _out_r   = RING.get('radius_m', 0.0) if FUEL_RING_ANY else 0.0
        _out_th  = RING.get('thickness_m', 0.0) if FUEL_RING_ANY else 0.0
        # Box area
        _A_box   = LX_M * LZ_M
        # Rod: π × (2.4σ)²
        _A_rod   = _np.pi * (2.4 * _rod_r)**2 if _rod_r > 0 else 0.0
        # Outer annulus: π × ((R+2σ)² − (R-2σ)²) = π × (4 × R × 2σ) = 8πR·σ
        if _out_r > 0 and _out_th > 0:
            _r_hi = _out_r + 2 * _out_th
            _r_lo = max(0.0, _out_r - 2 * _out_th)
            _A_out = _np.pi * (_r_hi**2 - _r_lo**2)
        else:
            _A_out = 0.0
        _A_base = max(0.0, _A_box - _A_rod - _A_out)
    except (NameError, KeyError, AttributeError):
        # Fallback if globals not in scope: equal weighting
        _A_box = 1.0; _A_rod = 0.0; _A_out = 0.0; _A_base = 1.0
    return _A_base, _A_rod, _A_out


def effective_target_density(sp_key):
    """Domain-area-weighted average density of target species seen by fast protons.

    Returns the spatial average of n_target(r,z) over the simulation box.
    This is the correct inline-diagnostic estimator for fusion rate in
    the limit that fast protons are randomly distributed.

    For zone-resolved accounting (correct per-region rates), use the
    first-transit analysis: pb11_first_transit_fusion.py.
    """
    A_base, A_rod, A_out = _zone_areas(sp_key)
    A_total = A_base + A_rod + A_out
    if A_total <= 0.0:
        return BASE['n_' + sp_key]  # degenerate fallback
    n_base = BASE['n_' + sp_key]
    n_rod  = ROD['n_'  + sp_key] if FUEL_ROD else 0.0
    n_ring = RING['n_' + sp_key] if FUEL_RING_ANY else 0.0
    return (A_base * n_base + A_rod * n_rod + A_out * n_ring) / A_total
...
if DIAG_PROFILE == 'preflight':
    ...
    # PATCHED: late_diag_period now aligns with --dump-period so the in-sim
    # fusion CSV writes one row per openPMD dump.  Previous hardcoded 5000
    # gave empty CSV for short UUF runs (n_steps < 5000).
    args.early_diag_period = 0     # DISABLED - was causing disk-full crashes
    args.early_diag_steps  = 0
    LATE_PERIOD_OVERRIDE   = 5000  # reverted: was breaking MPI in short runs
else:  # 'custom'
...
# v15.x: Keep total_time_s synchronised with the FINAL n_steps so run_meta.txt
# reports the true simulated duration. n_steps may have been modified by
# either the --max-steps override (line ~1233) or the preflight diag-profile
# cap (line ~1256). Without this recompute, run_meta inherits the LT-based
# total_time_s and reviewers see e.g. 7.72 ns reported for a run that
# actually simulated 1.16 ns — a 6.66× discrepancy vs reconnection_summary's
# time range. Invariant: total_time_s == n_steps × time_step_s.
...
if rank == 0:
    print('=' * 70)
    print('  p-11B/p-7Li/LiB RING RECONNECTION  v15.3 (pulsed + wind-and-release + schedule-file + drive-state log)')
    print('=' * 70)
    ...
    if IS_ROTATING:
        _rp = getattr(args, 'rotation_pattern', 'full')
        _n_drive_banner = {'full': N_SPOTS, 'two_pair': 4, 'single_pair': 2}[_rp]
        print(f'  External Jy:  rotational drive at {ROTATION_FREQ_HZ/1e6:.0f} MHz '
              f'(pattern={_rp}, {_n_drive_banner} active spots, '
              f'j_scale={args.j_scale:.3f})')
        if args.wind_and_release:
            t_drive_end_ps = args.charge_duration_ps + args.charge_ramp_out_ps
            print(f'  Drive mode:   WIND-AND-RELEASE')
            print(f'                charge {args.charge_duration_ps:.1f} ps  |  '
                  f'ramp-in {args.charge_ramp_in_ps:.1f} ps  |  '
                  f'ramp-out {args.charge_ramp_out_ps:.1f} ps')
            print(f'                drive permanently off after t={t_drive_end_ps:.1f} ps  |  '
                  f'release/observation phase begins')
        elif args.schedule_file:
            n_phases_b = len(_schedule_phases)
            n_cycles_b = max(p['cycle'] for p in _schedule_phases) if _schedule_phases else 0
            total_dur_ps_b = sum(p['duration_s'] for p in _schedule_phases) * 1e12
            print(f'  Drive mode:   SCHEDULE-FILE')
            print(f'                file: {args.schedule_file}')
            print(f'                {n_phases_b} phases  |  {n_cycles_b} cycles  |  '
                  f'total {total_dur_ps_b:.1f} ps')
        elif args.pulsed_drive:
            t_drive_end_ps = args.pulse_count * args.pulse_period_ps
            print(f'  Drive mode:   PULSED — {args.pulse_count} pulses '
                  f'({args.pulse_shape} envelope)')
            print(f'                period {args.pulse_period_ps:.1f} ps  |  '
                  f'width {args.pulse_width_ps:.1f} ps  |  '
                  f'duty cycle {100*args.pulse_width_ps/args.pulse_period_ps:.0f}%')
            print(f'                first pulse at t=0  |  '
                  f'drive permanently off after t={t_drive_end_ps:.0f} ps')
        else:
            print(f'  Drive mode:   CONTINUOUS (matches v13/v14)')
    else:
...
# AMReX arena pre-allocation — set explicitly to avoid OOM during
# diagnostic dumps with 8 ranks sharing a single GPU.
#
# Default behavior: each rank claims as much as the GPU has at init,
# leading to collisions and out-of-memory aborts under frequent
# dump_period.  With this cap each rank starts at 8 GB device,
# 8 ranks * 8 GB = 64 GB, leaving 17 GB headroom on H100's 81 GB.
# The arena can still grow on demand up to actual GPU capacity.
...
# ── v15: pulsed envelope construction ───────────────────────────────────────
def build_pulsed_envelope(pulse_count, pulse_period_s, pulse_width_s, shape='cos2'):
    """Construct a time-dependent envelope expression that turns ON for each
    pulse and OFF between pulses. After all pulse_count pulses have fired,
    the envelope is identically zero (drive permanently off).

    Parameters
    ----------
    pulse_count   : int, total number of pulses
    pulse_period_s: float, time between pulse starts (seconds)
    pulse_width_s : float, pulse FWHM duration (seconds)
    shape         : 'cos2', 'gauss', or 'square'

    Returns
    -------
    expr : str
        WarpX-compatible expression in terms of `t` that evaluates to 1.0
        at pulse peaks, 0.0 between pulses, and identically 0.0 after the
        Nth pulse completes.

    Implementation
    --------------
    Each pulse i is centered at t_i = i * pulse_period + pulse_width/2
    (so the first pulse PEAK is at t = pulse_width/2, but pulse STARTS at t=0).

    For 'cos2', within a pulse window [t_i - pulse_width/2, t_i + pulse_width/2]:
        env_i(t) = cos^2(pi * (t - t_i) / pulse_width)
    Outside the window: env_i(t) = 0 (enforced by stepfunction `heaviside`).

    Total envelope = sum_i env_i(t).

    NOTE: WarpX expression parser supports cos, exp, abs, heaviside, but not
    'if/else' or piecewise. We use heaviside(x) = 0 for x<0, 1 for x>=0
    to gate each pulse window.
    """
    # Hard cap: drive permanently off after the Nth pulse completes
    t_end_drive_s = pulse_count * pulse_period_s

    pulse_terms = []
    for i in range(pulse_count):
        t_center_s = i * pulse_period_s + pulse_width_s / 2.0
        t_start_s  = t_center_s - pulse_width_s / 2.0
        t_end_s    = t_center_s + pulse_width_s / 2.0

        # Gating: heaviside(t - t_start) * heaviside(t_end - t)
        # = 1 only inside [t_start, t_end], else 0
        gate = (f'(heaviside((t - {t_start_s:.8e}), 1.0) * '
                f'heaviside(({t_end_s:.8e} - t), 1.0))')

        if shape == 'cos2':
            # cos^2(pi*(t-tc)/W) — peaks at tc, zero at tc±W/2
            shape_expr = (f'(cos(3.141592653589793 * '
                          f'(t - {t_center_s:.8e}) / {pulse_width_s:.8e})**2)')
        elif shape == 'gauss':
            # Gaussian with sigma = W / (2*sqrt(2*ln(2)))
            sigma_s = pulse_width_s / (2.0 * np.sqrt(2.0 * np.log(2.0)))
            shape_expr = (f'(exp(-((t - {t_center_s:.8e})**2) / '
                          f'(2.0 * {sigma_s:.8e}**2)))')
        elif shape == 'square':
            shape_expr = '(1.0)'
        else:
        ...
        pulse_terms.append(f'({gate} * {shape_expr})')

    # After t_end_drive: drive permanently off
    # Simply by virtue of every pulse term being gated by heaviside(t_end_i - t),
    # the envelope is automatically 0 after the last pulse ends.
    envelope_expr = '(' + ' + '.join(pulse_terms) + ')'

    return envelope_expr


# ── v15.2: wind-and-release envelope ─────────────────────────────────────────
def build_wind_release_envelope(charge_duration_s, ramp_in_s, ramp_out_s):
    """Construct envelope for wind-and-release: single charge phase, then off.

    Phases (in seconds):
      [0, ramp_in_s]                   : cos^2 ramp-up (0 -> 1)
      [ramp_in_s, charge_duration_s]   : full amplitude (1)
      [charge_duration_s,
       charge_duration_s + ramp_out_s] : cos^2 ramp-down (1 -> 0)
      t > charge_duration_s + ramp_out_s : zero (release/observation phase)

    Returns a WarpX-compatible expression in `t`.

    The expression uses heaviside-gated piecewise terms, summed:
      env(t) = ramp_in_term + plateau_term + ramp_out_term

    Each piece is gated to its own time window via heaviside(); outside the
    window each piece evaluates to zero.
    """
    t1 = ramp_in_s                       # end of ramp-in
    t2 = charge_duration_s               # end of plateau (= start of ramp-out)
    t3 = charge_duration_s + ramp_out_s  # end of ramp-out (drive permanently off)

    # Ramp-in: cos^2 phase goes from -pi/2 (env=0) to 0 (env=1) as t: 0 -> ramp_in_s
    # phase_in(t) = -pi/2 + (pi/2) * (t / ramp_in_s) = (pi/2) * (t/ramp_in_s - 1)
    # env_in(t) = cos(phase_in)**2
    # Gated to [0, t1] via heaviside
    if ramp_in_s > 0:
        ramp_in_term = (
            f'((heaviside((t - 0.0), 1.0) * heaviside(({t1:.8e} - t), 1.0)) * '
            f'(cos(1.5707963267948966 * (t / {ramp_in_s:.8e} - 1.0))**2))'
        )
    else:
        # No ramp-in: jump straight to plateau (still need a 0-amplitude term
        # for syntactic uniformity, but make it harmless)
        ramp_in_term = '(0.0)'

    # Plateau: env = 1.0 for t in [t1, t2]
    plateau_term = (
        f'((heaviside((t - {t1:.8e}), 1.0) * heaviside(({t2:.8e} - t), 1.0)) * '
        f'(1.0))'
    )

    # Ramp-out: cos^2 phase goes from 0 (env=1) to pi/2 (env=0) as t: t2 -> t3
    # phase_out(t) = (pi/2) * (t - t2) / ramp_out_s
    # env_out(t) = cos(phase_out)**2
    if ramp_out_s > 0:
        ramp_out_term = (
            f'((heaviside((t - {t2:.8e}), 1.0) * heaviside(({t3:.8e} - t), 1.0)) * '
            f'(cos(1.5707963267948966 * (t - {t2:.8e}) / {ramp_out_s:.8e})**2))'
        )
    else:
        ramp_out_term = '(0.0)'

    # Total envelope = sum of phase terms (each non-zero only in its own window)
    envelope_expr = f'({ramp_in_term} + {plateau_term} + {ramp_out_term})'

    return envelope_expr


# ── v15.3: schedule-driven envelope (multi-phase) ────────────────────────────
def build_schedule_envelope(phases):
    """Build a WarpX-compatible expression for a multi-phase schedule.

    Generalises build_wind_release_envelope: each phase ramps from the
    previous phase's target to its own target over ramp_s, then holds at
    target for the remainder of duration_s. After the final phase ends, the
    envelope is identically zero (heaviside gating ensures this).

    The expression is a sum of heaviside-gated terms, each non-zero only in
    its own time window. Boundary overlaps at exact phase transitions can
    produce sub-timestep spikes of magnitude <= 2x; these are harmless in
    practice (no real timestep lands on the floating-point boundary).
    """
    PI_HALF = 1.5707963267948966
    terms = []
    t_cursor = 0.0
    v_prev   = 0.0  # envelope value before the very first phase
    ...
    for ph in phases:
        ...
        t_ramp_end    = t_cursor + r_s
        t_phase_end   = t_cursor + d_s

        # Ramp section: env = v_prev + (v_curr - v_prev) * sin^2(pi/2 * tau)
        #   tau = (t - t_phase_start) / r_s, tau in [0, 1].
        # At tau=0, env = v_prev. At tau=1, env = v_curr. Smooth (zero
        # derivative at endpoints; matches v15.2's cos^2 ramp shape).
        if r_s > 0:
            dv = v_curr - v_prev
            ramp_expr = (
                f'((heaviside((t - {t_phase_start:.8e}), 1.0) * '
                f'heaviside(({t_ramp_end:.8e} - t), 1.0)) * '
                f'({v_prev:.6f} + ({dv:.6f}) * '
                f'sin({PI_HALF} * (t - {t_phase_start:.8e}) / {r_s:.8e})**2))'
            )
            terms.append(ramp_expr)

        # Hold section: env = v_curr (constant) over [t_ramp_end, t_phase_end]
        # If hold duration is zero (r_s == d_s, all ramp), skip.
        # If v_curr == 0 (release-to-zero hold), skip - adds nothing.
        hold_duration = d_s - r_s
        if hold_duration > 0 and v_curr != 0.0:
            hold_expr = (
                f'((heaviside((t - {t_ramp_end:.8e}), 1.0) * '
                f'heaviside(({t_phase_end:.8e} - t), 1.0)) * '
                f'({v_curr:.6f}))'
            )
            terms.append(hold_expr)

        t_cursor = t_phase_end
        v_prev   = v_curr
...
def _eval_schedule_envelope(t_s, phases):
    """Python-side mirror of build_schedule_envelope for the drive log.

    Returns (env_value, cycle, phase_index, phase_type, time_in_phase_s).
    For t past the schedule end: returns
      (0.0, last_cycle, n_phases, 'post_schedule', t_s - schedule_end_s).
    """
    PI_HALF = np.pi / 2.0
    t_cursor = 0.0
    v_prev   = 0.0
    ...
    for ph in phases:
        ...
        if t_s < t_phase_end:
            t_in_phase = t_s - t_phase_start
            if t_in_phase < r_s and r_s > 0:
                env = v_prev + (v_curr - v_prev) * np.sin(PI_HALF * t_in_phase / r_s) ** 2
            else:
                env = v_curr
            return float(env), ph['cycle'], ph['phase_index'], ph['phase_type'], t_in_phase

        t_cursor = t_phase_end
        v_prev   = v_curr

    # Past schedule end: drive permanently off
    last = phases[-1]
    return 0.0, last['cycle'], len(phases), 'post_schedule', t_s - t_cursor
...
def build_Jy(rotating=False):
    ...
    # v15.3+: select active drive geometry. Pattern-symmetry comment:
    #   full        -> N_SPOTS-fold, every X-line driven simultaneously
    #   two_pair    -> 4-fold, two perpendicular diametric pairs sweep around ring
    #   single_pair -> 2-fold, one diametric pair sweeps around ring
    # X-line diagnostic positions are unchanged (between the 8 static nominal
    # positions); the drive sweeps through them as it rotates.
    rotation_pattern = getattr(args, 'rotation_pattern', 'full')
    if rotation_pattern == 'two_pair':
        n_drive    = 4
        angle_step = np.pi / 2.0
    elif rotation_pattern == 'single_pair':
        n_drive    = 2
        angle_step = np.pi
    else:  # 'full'
        n_drive    = N_SPOTS
        angle_step = 2.0 * np.pi / N_SPOTS
    ...
    base_jy = smooth_ramp() + ' * (' + ' + '.join(terms) + ')'

    # v15.2: wind-and-release envelope (single charge phase, then drive off).
    # Mutually exclusive with --pulsed-drive (enforced by argparse-level check).
    if args.wind_and_release and rotating:
        envelope = build_wind_release_envelope(
            charge_duration_s = args.charge_duration_ps  * 1e-12,
            ramp_in_s         = args.charge_ramp_in_ps   * 1e-12,
            ramp_out_s        = args.charge_ramp_out_ps  * 1e-12,
        )
        return f'({envelope}) * ({base_jy})'

    # v15.3: schedule-driven envelope (multi-phase, generalises wind-and-release).
    # Mutually exclusive with --pulsed-drive and --wind-and-release.
    if args.schedule_file and rotating:
        if not _schedule_phases:
            raise RuntimeError('--schedule-file specified but parsed schedule is empty')
        envelope = build_schedule_envelope(_schedule_phases)
        return f'({envelope}) * ({base_jy})'

    # v15: pulsed-drive envelope. When --pulsed-drive, multiply the rotating
    # Jy expression by a time-varying envelope that turns drive on/off in
    # cycles. Without this flag, drive is continuous (matches v13/v14).
    if args.pulsed_drive and rotating:
        envelope = build_pulsed_envelope(
            pulse_count    = args.pulse_count,
            pulse_period_s = args.pulse_period_ps * 1e-12,
            pulse_width_s  = args.pulse_width_ps  * 1e-12,
...
solver = picmi.HybridPICSolver(
    ...
    # PATCHED: substeps auto-scales with NX.
    # At 256² resolution 80 substeps is sufficient; at 512² the cell size
    # is half, so the CFL-limited substep dt is half, requiring at least
    # 160 substeps to avoid NaN in the E-field solver.
    substeps=(args.substeps if args.substeps > 0 else max(80, 80 * NX // 256)),
...
# ============================================================================
# v16: NUMPY-EVALUABLE SEED PROFILES (for runtime Biermann re-injection)
# ============================================================================
# The functions below mirror build_By_expression / build_vortex_field_expressions
# but operate on numpy (X, Z) arrays instead of returning WarpX expression
# strings. Used by the reseed callback to compute ΔB(x,z) on the grid each
# time the drive envelope crosses the rising-edge threshold.
#
# Two important design choices:
#
#   1. Peak amplitude is passed in explicitly (not B_SEED_T). The reseed
#      amplitude is independent of the t=0 seed amplitude — they answer
#      different physics questions. The t=0 seed sets the initial topology;
#      the reseed amplitude sets the per-pulse Biermann boost.
#
#   2. Same normalization convention as build_By_expression: the user-facing
#      amplitude equals the peak |B| achieved at r = sigma. This keeps the
#      numbers reported in event logs comparable to B_SEED_T.

def numpy_seed_harris(X, Z, amplitude_T):
    """Return ΔBy(X, Z) for the alternating-polarity Harris ring topology.

    Same spatial pattern as build_By_expression() but evaluated on a numpy
    grid. Each spot k contributes:
        sign_k * amplitude_T * (r_k / sigma) * exp(-0.5 * r_k^2 / sigma^2)
    where sign_k = (-1)^k. Peak |By| at r = sigma equals amplitude_T.

    Parameters
    ----------
    X, Z         : 2D numpy arrays giving cell-center x and z coordinates (m)
    amplitude_T  : float, peak |By| at r = sigma per spot (Tesla)

    Returns
    -------
    dBy : 2D numpy array, same shape as X and Z
    """
    sigma = SPOT_RADIUS_M
    dBy = np.zeros_like(X, dtype=np.float64)
    for k in range(N_SPOTS):
        angle = 2.0 * np.pi * k / N_SPOTS
        cx    = RING_RADIUS_M * np.cos(angle)
        cz    = RING_RADIUS_M * np.sin(angle)
        ...
        r2    = (X - cx) ** 2 + (Z - cz) ** 2
        r     = np.sqrt(r2)
        dBy  += sign * amplitude_T * (r / sigma) * np.exp(-0.5 * r2 / sigma**2)
    return dBy


def numpy_seed_vortex(X, Z, amplitude_T, same_handedness=True):
    """Return (ΔBx, ΔBz) for the in-plane vortex topology, evaluated on grid.

    Mirrors build_vortex_field_expressions:
        Bx = -s_k * A0 * (z - z_k)/sigma² * exp(-r²/2σ²)
        Bz = +s_k * A0 * (x - x_k)/sigma² * exp(-r²/2σ²)
    where A0 = amplitude_T * sigma * exp(1/2), so peak |B| at r=σ equals
    amplitude_T.

    Parameters
    ----------
    X, Z              : 2D numpy arrays of cell-center coordinates (m)
    amplitude_T       : float, peak |B| per spot (Tesla)
    same_handedness   : bool, True for physical Biermann (s_k = +1 all),
                        False for legacy alternating (s_k = (-1)^k)

    Returns
    -------
    dBx, dBz : 2D numpy arrays, same shape as X and Z
    """
    sigma  = SPOT_RADIUS_M
    sigma2 = sigma * sigma
    A0     = amplitude_T * sigma * np.exp(0.5)
    dBx = np.zeros_like(X, dtype=np.float64)
    dBz = np.zeros_like(X, dtype=np.float64)
    for k in range(N_SPOTS):
        angle = 2.0 * np.pi * k / N_SPOTS
        cx    = RING_RADIUS_M * np.cos(angle)
        cz    = RING_RADIUS_M * np.sin(angle)
        sk    = 1.0 if same_handedness else (-1.0) ** k
        r2    = (X - cx) ** 2 + (Z - cz) ** 2
        gauss = np.exp(-0.5 * r2 / sigma2)
        dBx  += -sk * A0 * (Z - cz) / sigma2 * gauss
        dBz  +=  sk * A0 * (X - cx) / sigma2 * gauss
    return dBx, dBz


def numpy_density_ratio(X, Z):
    """Return n_e(x,z)/N_PEAK_M3 evaluated from the analytic ring density.

    Uses the same Gaussian-spot pattern that initialized the plasma, so the
    coupling factor is exactly 1.0 at spot peaks and falls to background/
    N_PEAK_M3 far from the ring. Returns values in [N_BG/N_PEAK, 1.0+ε].

    NOTE: This does NOT track live plasma depletion between pulses. A more
    physical version would read the actual particle density from WarpX
    every reseed event; planned for v17 if v16 calibration sweep indicates
    the reseed mechanism is doing real work.
    """
    sigma2 = SPOT_RADIUS_M ** 2
    spot_sum = np.zeros_like(X, dtype=np.float64)
    for k in range(N_SPOTS):
        angle = 2.0 * np.pi * k / N_SPOTS
        cx    = RING_RADIUS_M * np.cos(angle)
        cz    = RING_RADIUS_M * np.sin(angle)
        r2    = (X - cx) ** 2 + (Z - cz) ** 2
        spot_sum += np.exp(-0.5 * r2 / sigma2)
    # n_e(x,z) = N_BG + N_PEAK * sum(Gaussians)
    # ratio = (N_BG + N_PEAK * sum) / N_PEAK = N_BG/N_PEAK + sum
    return (N_BACKGROUND_M3 / N_PEAK_M3) + spot_sum


def get_reseed_topology():
    """Resolve --biermann-reseed-topology, honoring 'match-seed' alias."""
    requested = getattr(args, 'biermann_reseed_topology', 'match-seed')
    if requested == 'match-seed':
        return getattr(args, 'seed_topology', 'harris')
    return requested
...
def write_fusion_diag_header():
    ...
    header = (
        ...
        # FIX F (RETIRED): centre/outer zone columns dropped — see Methods.
        # The legacy R_CENTRE=0.25*R_RING diagnostic doesn't align with the
        # 8-spot reconnection X-line annulus.  Use zone analysis (Stage D)
        # and consolidate_reports (Stage 2) for canonical zone diagnostics.
    )
...
def write_fusion_accounting():
    ...
    with open(FUSION_ACCOUNTING_FILE, 'w', encoding='utf-8') as fh:
        ...
        # PATCHED: report 'none' when region's fuel is disabled
        eff_rod  = args.rod_fuel  if FUEL_ROD else 'none'
        eff_ring = args.ring_fuel if FUEL_RING_ANY else 'none'
        fh.write(f'base_fuel={args.base_fuel}  '
                 f'rod_fuel={eff_rod}  '
                 f'ring_fuel={eff_ring}\n')
        fh.write('# NOTE: rod_fuel and ring_fuel show "none" when the\n')
        fh.write('#       corresponding region is disabled by --fuel-*\n')
        fh.write('#       flags. Underlying argparse defaults preserved\n')
        fh.write('#       only in the simulation namespace.\n')
...
def fusion_diagnostic_callback():
    ...
    # SKIP final-step callback to avoid MPI deadlock.
    # Final-step particle access can fail asymmetrically across ranks
    # (deprecated API + finalization-phase particle migration), causing
    # comm.allreduce in succeeding ranks to wait forever for failed ranks.
    # 75 prior dumps cover the trajectory; final openPMD dump captures state.
    if step == n_steps:
        return
    ...
    # MPI-SAFE: convert per-rank particle access failure into 'empty arrays'.
    # This ensures every rank participates in EVERY allreduce below, even
    # when a rank has no local particles or the deprecated API returned None.
    # Without this, one failing rank skips the collective and the remaining
    # ranks block forever waiting for the failed rank to contribute.
    if ke_kev is None or weights is None:
        # Log the failure mode (only rank 0, only once per distinct mode)
        ...
        if rank == 0 and status not in seen:
            ...
            print(f'  [fusion_diag] step={step} particle access FAILED: {status}',
                  flush=True)
        # Replace None with empty arrays so the unified path below works
        ke_kev  = np.array([], dtype=float)
        weights = np.array([], dtype=float)

    # UNIFIED PATH: every rank executes every allreduce, contributing
    # either real values or 0 (when local arrays are empty).
    wt    = comm.allreduce(float(np.sum(weights)) if len(weights) > 0 else 0.0,
                           op=mpi.SUM)
    ...
    for rxn in ALL_RXNS:
        if rxn['active']:
            # Local contribution (zero when this rank has no particles)
            if len(weights) > 0:
                fast_mask  = ke_kev >= rxn['threshold_kev']
                wf_local   = float(np.sum(weights[fast_mask]))
            ...
            # All ranks participate in these allreduces
            wf               = comm.allreduce(wf_local,          op=mpi.SUM)
            ...
            alphas_step[rxn['key'] + '_sigma'] = rate_sigma_total * rxn['n_alphas'] * delta_t
        else:
    ...
    # FIX F (RETIRED): centre/outer metrics dropped.  The legacy
    ...
    if rank == 0:
        row = (
            ...
            # FIX F (RETIRED): centre/outer metrics dropped — see Stage D zone analysis.
        )
...
# ============================================================================
# v15.1: PULSED DRIVE STATE LOG
# ============================================================================
# When --pulsed-drive is active, log the drive envelope value at fine cadence
# (every 10 steps = ~1.5 ps) so we can correlate drive-on/off windows with
# fusion-rate spikes from the inline fusion diagnostic. Used to verify whether
# pulses align with reconnection events.
#
# Output: <outdir>/drive_state_log.csv with columns:
#   step, t_ps, drive_envelope, drive_active, pulse_index,
#   time_in_pulse_ps, time_to_next_pulse_ps
#
# The envelope is computed analytically in Python from the same parameters
# used to build the WarpX expression. Since both use the same formula, the
# logged envelope matches what WarpX is applying to Jy.

if args.pulsed_drive and IS_ROTATING:
    DRIVE_LOG_FILE     = os.path.join(OUTDIR, 'drive_state_log.csv')
    DRIVE_LOG_INTERVAL = 10   # every 10 steps ≈ 1.5 ps cadence
    PULSE_PERIOD_S     = args.pulse_period_ps * 1e-12
    PULSE_WIDTH_S      = args.pulse_width_ps  * 1e-12
    PULSE_COUNT        = args.pulse_count
    PULSE_SHAPE        = args.pulse_shape
    PULSE_END_S        = PULSE_COUNT * PULSE_PERIOD_S
    ...
    def _eval_envelope(t_s):
        """Evaluate the pulse envelope at time t_s (matches WarpX formula).

        Returns (envelope_value, pulse_index, time_in_pulse_s).
        pulse_index = -1 if outside any pulse window.
        """
        # Beyond the last pulse: envelope identically zero
        if t_s >= PULSE_END_S:
            return 0.0, -1, 0.0

        # Find which pulse window we might be in
        # Pulse i spans [i * period, i * period + width]
        i = int(t_s / PULSE_PERIOD_S)
        if i >= PULSE_COUNT:
            return 0.0, -1, 0.0
        t_start_s   = i * PULSE_PERIOD_S
        t_end_s     = t_start_s + PULSE_WIDTH_S
        t_center_s  = t_start_s + PULSE_WIDTH_S / 2.0

        if t_s < t_start_s or t_s > t_end_s:
            # Between pulses
            return 0.0, -1, 0.0

        # Inside pulse window
        if PULSE_SHAPE == 'cos2':
            phase = np.pi * (t_s - t_center_s) / PULSE_WIDTH_S
            env = np.cos(phase) ** 2
        elif PULSE_SHAPE == 'gauss':
            sigma_s = PULSE_WIDTH_S / (2.0 * np.sqrt(2.0 * np.log(2.0)))
            env = np.exp(-((t_s - t_center_s) ** 2) / (2.0 * sigma_s ** 2))
        elif PULSE_SHAPE == 'square':
            env = 1.0
        else:
            env = 0.0

        time_in_pulse_s = t_s - t_start_s
        return float(env), i, time_in_pulse_s

    def drive_state_log_callback():
        """Log drive envelope value at fine cadence for alignment analysis."""
        try:
            step = simulation.extension.warpx.getistep(lev=0)
        except Exception:
            return
        if step % DRIVE_LOG_INTERVAL != 0:
            return
        if _drive_log_state['last_step'] == step:
            return
        _drive_log_state['last_step'] = step
        ...
        # Header
        if not _drive_log_state['header_written']:
            with open(DRIVE_LOG_FILE, 'w', newline='', encoding='utf-8') as fh:
                fh.write('step,t_ps,drive_envelope,drive_active,pulse_index,'
                         'time_in_pulse_ps,time_to_next_pulse_ps\n')
            _drive_log_state['header_written'] = True

        env, pulse_idx, t_in_pulse_s = _eval_envelope(t_s)
        drive_active = 1 if env > 0.01 else 0

        # Time to start of next pulse (useful for alignment analysis)
        if pulse_idx >= 0:
            # Currently in a pulse — time to next is start of next pulse
            next_pulse_start_s = (pulse_idx + 1) * PULSE_PERIOD_S
        else:
            # Between pulses — find the next pulse start
            i_next = int(np.ceil(t_s / PULSE_PERIOD_S))
            if i_next >= PULSE_COUNT:
                next_pulse_start_s = float('nan')
            else:
                next_pulse_start_s = i_next * PULSE_PERIOD_S
        t_to_next_s = (next_pulse_start_s - t_s) if not np.isnan(next_pulse_start_s) else float('nan')

        with open(DRIVE_LOG_FILE, 'a', newline='', encoding='utf-8') as fh:
            fh.write(f'{step},{t_ps:.6e},{env:.6e},{drive_active},{pulse_idx},'
                     f'{t_in_pulse_s*1e12:.6e},{t_to_next_s*1e12:.6e}\n')

    callbacks.installafterstep(drive_state_log_callback)
    if rank == 0:
        print(f'  v15.1: drive-state log ENABLED')
        print(f'         log interval: every {DRIVE_LOG_INTERVAL} steps '
              f'(~{DRIVE_LOG_INTERVAL * time_step_s * 1e12:.1f} ps)')
        print(f'         output: {DRIVE_LOG_FILE}')


# v15.2 wind-and-release drive log: parallel to v15.1 pulsed log.
# Logs the envelope at fine cadence so we can correlate the charge/release
# transition with E95_core dynamics.
#
# CSV columns: step, t_ps, drive_envelope, drive_active, phase
#   phase = 'ramp_in' | 'plateau' | 'ramp_out' | 'release'



# ── v15.3: schedule-driven drive log ─────────────────────────────────────────
# Logs the schedule envelope at fine cadence with cycle and phase metadata.
# Output: <outdir>/drive_state_log.csv with columns:
#   step, t_ps, drive_envelope, drive_active, cycle, phase_index, phase_type, time_in_phase_ps
# Mutually exclusive with v15 pulsed-drive and v15.2 wind-and-release drive logs
# (enforced at argparse level — only one drive mode active per run).

if args.schedule_file and IS_ROTATING:
    SCHED_DRIVE_LOG_FILE     = os.path.join(OUTDIR, 'drive_state_log.csv')
    SCHED_DRIVE_LOG_INTERVAL = 10   # every 10 steps, matches v15.2 cadence
    ...
    def sched_drive_state_log_callback():
        """Log schedule envelope at fine cadence."""
        try:
            step = simulation.extension.warpx.getistep(lev=0)
        except Exception:
            return
        if step % SCHED_DRIVE_LOG_INTERVAL != 0:
            return
        if _sched_log_state['last_step'] == step:
            return
        _sched_log_state['last_step'] = step
        ...
        if not _sched_log_state['header_written']:
            with open(SCHED_DRIVE_LOG_FILE, 'w', newline='', encoding='utf-8') as fh:
                fh.write('step,t_ps,drive_envelope,drive_active,'
                         'cycle,phase_index,phase_type,time_in_phase_ps\n')
            _sched_log_state['header_written'] = True

        env, cycle, phase_index, phase_type, t_in_phase_s = _eval_schedule_envelope(
            t_s, _schedule_phases
        )
        drive_active = 1 if env > 0.01 else 0

        with open(SCHED_DRIVE_LOG_FILE, 'a', newline='', encoding='utf-8') as fh:
            fh.write(f'{step},{t_ps:.6e},{env:.6e},{drive_active},'
                     f'{cycle},{phase_index},{phase_type},'
                     f'{t_in_phase_s*1e12:.6e}\n')

    callbacks.installafterstep(sched_drive_state_log_callback)
    if rank == 0:
        n_phases = len(_schedule_phases)
        n_cycles = max(p['cycle'] for p in _schedule_phases) if _schedule_phases else 0
        total_dur_ps = sum(p['duration_s'] for p in _schedule_phases) * 1e12
        print(f'  v15.3: schedule-driven drive log ENABLED')
        print(f'         schedule: {n_phases} phases, {n_cycles} cycle(s), '
              f'total {total_dur_ps:.1f} ps')
        print(f'         log interval: every {SCHED_DRIVE_LOG_INTERVAL} steps '
              f'(~{SCHED_DRIVE_LOG_INTERVAL * time_step_s * 1e12:.1f} ps)')


if args.wind_and_release and IS_ROTATING:
    WR_DRIVE_LOG_FILE     = os.path.join(OUTDIR, 'drive_state_log.csv')
    WR_DRIVE_LOG_INTERVAL = 10   # every 10 steps ≈ 1.5 ps cadence
    WR_RAMP_IN_S          = args.charge_ramp_in_ps   * 1e-12
    WR_CHARGE_S           = args.charge_duration_ps  * 1e-12
    WR_RAMP_OUT_S         = args.charge_ramp_out_ps  * 1e-12
    WR_DRIVE_END_S        = WR_CHARGE_S + WR_RAMP_OUT_S
    ...
    def _eval_wr_envelope(t_s):
        """Evaluate wind-and-release envelope at time t_s.

        Returns (envelope_value, phase_label, time_in_phase_s).
        Matches the WarpX expression in build_wind_release_envelope().
        """
        # Phase 1: ramp-in [0, ramp_in_s]
        if t_s < WR_RAMP_IN_S:
            if WR_RAMP_IN_S > 0:
                phase_arg = (np.pi / 2.0) * (t_s / WR_RAMP_IN_S - 1.0)
                env = np.cos(phase_arg) ** 2
            else:
                env = 1.0
            return float(env), 'ramp_in', t_s

        # Phase 2: plateau [ramp_in_s, charge_s]
        if t_s < WR_CHARGE_S:
            return 1.0, 'plateau', (t_s - WR_RAMP_IN_S)

        # Phase 3: ramp-out [charge_s, drive_end_s]
        if t_s < WR_DRIVE_END_S:
            if WR_RAMP_OUT_S > 0:
                phase_arg = (np.pi / 2.0) * (t_s - WR_CHARGE_S) / WR_RAMP_OUT_S
                env = np.cos(phase_arg) ** 2
            else:
                env = 0.0
            return float(env), 'ramp_out', (t_s - WR_CHARGE_S)

        # Phase 4: release (drive permanently off)
        return 0.0, 'release', (t_s - WR_DRIVE_END_S)

    def wr_drive_state_log_callback():
        """Log wind-and-release envelope at fine cadence."""
        try:
            step = simulation.extension.warpx.getistep(lev=0)
        except Exception:
            return
        if step % WR_DRIVE_LOG_INTERVAL != 0:
            return
        if _wr_log_state['last_step'] == step:
            return
        _wr_log_state['last_step'] = step
        ...
        if not _wr_log_state['header_written']:
            with open(WR_DRIVE_LOG_FILE, 'w', newline='', encoding='utf-8') as fh:
                fh.write('step,t_ps,drive_envelope,drive_active,phase,time_in_phase_ps\n')
            _wr_log_state['header_written'] = True

        env, phase_label, t_in_phase_s = _eval_wr_envelope(t_s)
        drive_active = 1 if env > 0.01 else 0

        with open(WR_DRIVE_LOG_FILE, 'a', newline='', encoding='utf-8') as fh:
            fh.write(f'{step},{t_ps:.6e},{env:.6e},{drive_active},'
                     f'{phase_label},{t_in_phase_s*1e12:.6e}\n')

    callbacks.installafterstep(wr_drive_state_log_callback)
    if rank == 0:
        print(f'  v15.2: wind-and-release drive log ENABLED')
        print(f'         log interval: every {WR_DRIVE_LOG_INTERVAL} steps '
              f'(~{WR_DRIVE_LOG_INTERVAL * time_step_s * 1e12:.1f} ps)')
        print(f'         output: {WR_DRIVE_LOG_FILE}')


# ============================================================================
# v16: BIERMANN BATTERY RE-SEED ON DRIVE RISING-EDGE EVENTS
# ============================================================================
# When --biermann-reseed is set AND a drive envelope is active, install an
# after-step callback that:
#   1. Tracks the drive envelope value at every step
#   2. Detects rising-edge events (envelope crosses threshold upward, with
#      cycle-locked single-fire per pulse/cycle)
#   3. On each event, computes ΔBy(x,z) on the simulation grid using the
#      numpy-evaluable seed profile, optionally density-coupled
#   4. Writes ΔBy into the WarpX By MultiFab via the pywarpx fields wrapper
#   5. Logs the event to biermann_reseed_events.csv
#
# CRITICAL: this feature requires write access to the field MultiFab from
# Python. The startup probe verifies whether the pywarpx field-write API is
# usable for this build before any reseed events fire. If the probe fails,
# the feature self-disables and the run proceeds as v15.

if args.biermann_reseed and IS_ROTATING:
    # Determine which drive mode is active. v16 supports all three.
    _RESEED_DRIVE_MODE = None
    if args.pulsed_drive:
        _RESEED_DRIVE_MODE = 'pulsed'
    elif args.wind_and_release:
        _RESEED_DRIVE_MODE = 'wind_release'
    elif args.schedule_file:
        _RESEED_DRIVE_MODE = 'schedule'
    else:
        if rank == 0:
            print('  v16: --biermann-reseed requires --pulsed-drive, '
                  '--wind-and-release, or --schedule-file. DISABLED.')

    if _RESEED_DRIVE_MODE is not None:
        RESEED_LOG_FILE       = os.path.join(OUTDIR, 'biermann_reseed_events.csv')
        RESEED_AMPLITUDE_T    = args.biermann_reseed_amplitude_T
        RESEED_THRESHOLD      = args.biermann_reseed_threshold
        RESEED_RESET_THRESH   = 0.05  # env must drop below this to re-arm
        RESEED_DENSITY_COUPLE = args.biermann_reseed_density_couple
        RESEED_TOPOLOGY       = get_reseed_topology()

        _reseed_state = {
            'prev_env'         : 0.0,
            'prev_cycle'       : -2,    # ensure first event fires
            'last_reseed_cyc'  : -1,
            'armed'            : True,  # True when ready to fire on next rising edge
            'count'            : 0,
            'header_written'   : False,
            'write_available'  : None,  # set by probe; None = not yet probed
            'last_step_seen'   : -1,
            'field_wrapper'    : None,  # cached wrapper, populated by probe
        }

        # ------------------------------------------------------------------
        # Field-write probe: verify pywarpx allows in-place B-field writes.
        # Tries the modern fields.ByWrapper() API first, falls back to the
        # legacy getter pattern, and gracefully self-disables if neither works.
        # Runs ONCE at the start of the first callback invocation (we cannot
        # probe before simulation.step has been called at least once because
        # the MultiFab is not allocated until the first step).
        # ------------------------------------------------------------------
        def _probe_field_write_access():
            """Return ('wrapper', wrapper_obj) or ('getter', None) or ('none', reason)."""
            # Try modern wrapper API first
            try:
                from pywarpx import fields as _wxfields
                if hasattr(_wxfields, 'ByWrapper'):
                    wrap = _wxfields.ByWrapper()
                    # Try a no-op write: read a value, write same value back
                    arr_test = wrap[...]
                    if arr_test is None or np.asarray(arr_test).size == 0:
                        return ('none', 'wrapper returned empty array')
                    # Try writing the same data back (no physical change)
                    # If this succeeds, in-place modification is supported.
                    try:
                        wrap[...] = arr_test
                        return ('wrapper', wrap)
                    except Exception as e:
                        return ('none', f'wrapper write failed: {type(e).__name__}: {e}')
            except ImportError:
                pass
            except Exception as e:
                pass  # fall through to getter fallback

            # Fallback: try in-place modification via the getter, verify by re-read
            try:
                wx = simulation.extension.warpx
                if not hasattr(wx, 'getBy'):
                    return ('none', 'no getBy method on warpx extension')
                arr_before = wx.getBy(lev=0, include_ghost=False)
                if isinstance(arr_before, (list, tuple)) and arr_before:
                    arr_before = arr_before[0]
                arr_before = np.asarray(_to_cpu(arr_before)) if '_to_cpu' in globals() else np.asarray(arr_before)
                # Note: we can't reliably test write-back via getter without
                # risking corruption. Treat as 'getter' mode but flag as
                # untested. The first real reseed will tell us if it took.
                return ('getter', None)
            except Exception as e:
                return ('none', f'getter probe failed: {type(e).__name__}: {e}')

        # ------------------------------------------------------------------
        # Envelope evaluation: unified across drive modes.
        # Returns (env_value, cycle_id) where cycle_id increments per pulse
        # or per schedule cycle. cycle_id = -1 means no active cycle.
        # ------------------------------------------------------------------
        def _eval_drive_for_reseed(t_s):
            if _RESEED_DRIVE_MODE == 'pulsed':
                env, pulse_idx, _ = _eval_envelope(t_s)
                return float(env), int(pulse_idx)
            elif _RESEED_DRIVE_MODE == 'wind_release':
                env, phase_label, _ = _eval_wr_envelope(t_s)
                # Treat the single charge phase as cycle 0; ramp_out / release
                # do not get reseeded again. Use a synthetic cycle ID based on
                # phase.
                cycle_id = 0 if phase_label in ('ramp_in', 'plateau') else -1
                return float(env), cycle_id
            elif _RESEED_DRIVE_MODE == 'schedule':
                env, cycle, _, _, _ = _eval_schedule_envelope(t_s, _schedule_phases)
                return float(env), int(cycle)
            return 0.0, -1

        # ------------------------------------------------------------------
        # Build ΔB(x, z) for an event. Returns (dBx, dBy, dBz) numpy arrays
        # shaped to match the local field view (which is currently assumed
        # to be the full domain on rank 0; see comment in _try_get_field_array).
        # ------------------------------------------------------------------
        def _build_reseed_field(grid_shape):
            """Construct ΔBx, ΔBy, ΔBz for one reseed event.

            grid_shape: (nx, nz) of the field view we'll write into.
            Returns (dBx, dBy, dBz, density_ratio_max) — three same-shape
            numpy arrays of dtype float64, plus the peak density ratio
            for diagnostic logging.
            """
            nx, nz = grid_shape
            lo_x = grid.lower_bound[0]; hi_x = grid.upper_bound[0]
            lo_z = grid.lower_bound[1]; hi_z = grid.upper_bound[1]
            dx = (hi_x - lo_x) / nx
            dz = (hi_z - lo_z) / nz
            # Cell-center coordinates
            x_centers = lo_x + (np.arange(nx) + 0.5) * dx
            z_centers = lo_z + (np.arange(nz) + 0.5) * dz
            X, Z = np.meshgrid(x_centers, z_centers, indexing='ij')

            # Topology dispatch
            if RESEED_TOPOLOGY == 'harris':
                dBy = numpy_seed_harris(X, Z, RESEED_AMPLITUDE_T)
                dBx = np.zeros_like(dBy)
                dBz = np.zeros_like(dBy)
            elif RESEED_TOPOLOGY == 'vortex_same':
                dBx, dBz = numpy_seed_vortex(X, Z, RESEED_AMPLITUDE_T,
                                             same_handedness=True)
                dBy = np.zeros_like(dBx)
            elif RESEED_TOPOLOGY == 'vortex_alt':
                dBx, dBz = numpy_seed_vortex(X, Z, RESEED_AMPLITUDE_T,
                                             same_handedness=False)
                dBy = np.zeros_like(dBx)
            else:
                raise ValueError(f'Unknown reseed topology: {RESEED_TOPOLOGY}')

            # Density coupling
            if RESEED_DENSITY_COUPLE == 'analytic':
                ratio = numpy_density_ratio(X, Z)
            ...
            return dBx, dBy, dBz, ratio_max

        # ------------------------------------------------------------------
        # Write ΔB into the WarpX field arrays. Returns ('ok', None) on
        # success, ('fail', reason) on any failure. Never raises.
        # ------------------------------------------------------------------
        def _apply_reseed_to_field(dBx, dBy, dBz):
            mode = _reseed_state['write_available']
            wrap = _reseed_state['field_wrapper']

            try:
                if mode == 'wrapper' and wrap is not None:
                    # Modern API: wrap[...] += delta
                    from pywarpx import fields as _wxfields
                    by_w = _wxfields.ByWrapper()
                    arr  = by_w[...]
                    arr_np = np.asarray(arr)
                    # Handle shape mismatch (we may have built a wrong-shape grid)
                    if arr_np.shape != dBy.shape:
                        return ('fail',
                                f'By shape mismatch: field {arr_np.shape} vs '
                                f'computed {dBy.shape}')
                    by_w[...] = arr_np + dBy
                    # Bx and Bz only relevant for vortex topologies
                    if dBx.any():
                        bx_w = _wxfields.BxWrapper()
                        bx_w[...] = np.asarray(bx_w[...]) + dBx
                    if dBz.any():
                        bz_w = _wxfields.BzWrapper()
                        bz_w[...] = np.asarray(bz_w[...]) + dBz
                    return ('ok', None)
                elif mode == 'getter':
                    # Legacy in-place modification via getter
                    wx = simulation.extension.warpx
                    by_arr = wx.getBy(lev=0, include_ghost=False)
                    if isinstance(by_arr, (list, tuple)) and by_arr:
                        by_arr = by_arr[0]
                    by_np = np.asarray(by_arr)
                    if by_np.shape != dBy.shape:
                        return ('fail',
                                f'By shape mismatch: field {by_np.shape} vs '
                                f'computed {dBy.shape}')
                    # In-place add; may or may not propagate depending on view
                    by_np += dBy
                    return ('ok', 'getter (in-place, view propagation untested)')
                else:
                    return ('fail', 'no write mode available')
            except Exception as e:
                return ('fail', f'{type(e).__name__}: {e}')

        # ------------------------------------------------------------------
        # The main callback. One state-machine pass per step.
        # ------------------------------------------------------------------
        def biermann_reseed_callback():
            try:
                step = simulation.extension.warpx.getistep(lev=0)
            except Exception:
                return
            if _reseed_state['last_step_seen'] == step:
                return
            _reseed_state['last_step_seen'] = step

            # First-time probe (deferred until first callback invocation so the
            # MultiFab is allocated). Rank 0 probes and broadcasts result.
            if _reseed_state['write_available'] is None:
                if rank == 0:
                    mode, payload = _probe_field_write_access()
                    _reseed_state['write_available'] = mode
                    if mode == 'wrapper':
                        _reseed_state['field_wrapper'] = payload
                        print(f'  v16 probe: field-write access OK via fields.ByWrapper()')
                    elif mode == 'getter':
                        print(f'  v16 probe: field-write access via legacy getter '
                              f'(in-place modification — propagation untested)')
                    else:
                        print(f'  v16 probe: field-write access UNAVAILABLE — '
                              f'reseed will be LOGGED but not APPLIED. Reason: {payload}')
                # Note: in multi-rank runs, the probe result needs broadcasting.
                # For now, only rank 0 actually writes (matching pattern in
                # _try_get_field_array). Other ranks just skip the write.

            # All envelope tracking happens at every step; the heavy lifting
            # (field write) only on rising-edge events.
            t_s   = step * time_step_s
            t_ps  = t_s * 1e12
            env, cycle_id = _eval_drive_for_reseed(t_s)

            # Re-arm condition: when envelope drops well below threshold,
            # we're ready to fire again on the next rising edge.
            if env < RESEED_RESET_THRESH:
                _reseed_state['armed'] = True

            # Rising-edge detection: armed AND envelope crosses threshold upward
            # AND we have not already fired in this cycle.
            rising_edge = (
                _reseed_state['armed']
                and _reseed_state['prev_env'] < RESEED_THRESHOLD
                and env >= RESEED_THRESHOLD
                and cycle_id >= 0
                and cycle_id != _reseed_state['last_reseed_cyc']
            )

            if rising_edge:
                # Fire. Only rank 0 applies the write (consistent with
                # _try_get_field_array assumption that rank 0 sees the
                # full domain in unified-memory builds).
                write_status = 'skipped (non-rank-0)'
                peak_dB = 0.0
                ratio_max = 0.0

                if rank == 0:
                    # Determine field grid shape. Use the WarpX getter to
                    # find the actual local view shape.
                    try:
                        if _reseed_state['write_available'] == 'wrapper':
                            from pywarpx import fields as _wxfields
                            arr = np.asarray(_wxfields.ByWrapper()[...])
                            grid_shape = arr.shape
                        else:
                            wx = simulation.extension.warpx
                            arr = wx.getBy(lev=0, include_ghost=False)
                            if isinstance(arr, (list, tuple)) and arr:
                                arr = arr[0]
                            grid_shape = np.asarray(arr).shape
                    except Exception as e:
                        grid_shape = None
                        write_status = f'fail (shape query): {e}'

                    if grid_shape is not None and len(grid_shape) == 2:
                        try:
                            dBx, dBy, dBz, ratio_max = _build_reseed_field(grid_shape)
                            peak_dB = float(np.max(np.sqrt(dBx**2 + dBy**2 + dBz**2)))
                            if RESEED_AMPLITUDE_T > 0:
                                if _reseed_state['write_available'] in ('wrapper', 'getter'):
                                    status, reason = _apply_reseed_to_field(dBx, dBy, dBz)
                                    write_status = 'ok' if status == 'ok' else f'fail: {reason}'
                                else:
                                    write_status = 'logged-only (no write access)'
                            else:
                                write_status = 'amplitude=0 (logged-only)'
                        except Exception as e:
                            write_status = f'fail (build): {type(e).__name__}: {e}'
                    elif grid_shape is None:
                        pass  # already set above
                    else:
                        write_status = f'fail (unexpected grid shape: {grid_shape})'

                    # Log event
                    if not _reseed_state['header_written']:
                        with open(RESEED_LOG_FILE, 'w', newline='', encoding='utf-8') as fh:
                            fh.write('step,t_ps,cycle_id,drive_env_at_trigger,'
                                     'reseed_amplitude_T,peak_dB_T_injected,'
                                     'analytic_density_ratio_max,cumulative_reseed_count,'
                                     'write_status\n')
                        _reseed_state['header_written'] = True

                    _reseed_state['count'] += 1
                    with open(RESEED_LOG_FILE, 'a', newline='', encoding='utf-8') as fh:
                        fh.write(f'{step},{t_ps:.6e},{cycle_id},{env:.6e},'
                                 f'{RESEED_AMPLITUDE_T:.4f},{peak_dB:.6e},'
                                 f'{ratio_max:.6e},{_reseed_state["count"]},'
                                 f'{write_status}\n')

                # Update state to prevent re-firing this cycle
                _reseed_state['armed'] = False
                _reseed_state['last_reseed_cyc'] = cycle_id

            _reseed_state['prev_env'] = env
            _reseed_state['prev_cycle'] = cycle_id

        callbacks.installafterstep(biermann_reseed_callback)

        if rank == 0:
            print(f'  v16: BIERMANN BATTERY RE-SEED ENABLED')
            print(f'       drive mode:       {_RESEED_DRIVE_MODE}')
            print(f'       amplitude:        {RESEED_AMPLITUDE_T:.1f} T per pulse')
            print(f'       threshold:        env > {RESEED_THRESHOLD:.2f} '
                  f'(re-arm at env < {RESEED_RESET_THRESH:.2f})')
            print(f'       density coupling: {RESEED_DENSITY_COUPLE}')
            print(f'       topology:         {RESEED_TOPOLOGY}')
            print(f'       event log:        {RESEED_LOG_FILE}')
            if args.biermann_reseed_write_probe_only:
                print(f'       MODE: WRITE-PROBE ONLY — will exit after probe')
...
if rank == 0:
    with open(META_FILE, 'w', encoding='utf-8') as fh:
        fh.write('v16.0 per-region fuel config + Biermann reseed run metadata\n' + '='*60 + '\n')
        fh.write(f'version=16.0\ntimestamp={datetime.now().isoformat()}\n')
        fh.write(f'outdir={OUTDIR}\n')
        fh.write('\n[geometry]\n')
        fh.write(f'n_spots={N_SPOTS}\n')
        fh.write(f'rotation_pattern={getattr(args, "rotation_pattern", "full")}\n')
        ...
        if FUEL_ROD:
            fh.write(f'rod_radius_m={ROD_RADIUS_M:.3e}\n')
        if FUEL_RING_ANY:
            fh.write(f'outer_radius_m={OUTER_RADIUS_M:.3e}\n')
            fh.write(f'outer_thickness_m={OUTER_THICKNESS_M:.3e}\n')
        fh.write(f'lx_m={LX_M:.3e}\n')
        fh.write(f'lz_m={LZ_M:.3e}\n')
        fh.write('\n[fuel_regions]\n')
        # v15.x: When a region is disabled, write fuel='none' rather than the
        # default preset's name. The fusion_accounting_notes.txt writer
        # (~line 2295) already does this correctly; this brings run_meta.txt
        # into line. Without this, run_meta reads e.g. "rod_fuel=ammonia_borane
        # enabled=False" which is correct strictly (the 'enabled=False' flag is
        # operative) but invites reviewer confusion. The density/composition
        # fields are still written for completeness, but report the would-be
        # values explicitly so disabled regions are unambiguous.
        for region, cfg, flag, is_enabled in [('base',BASE,'always',         True),
                                               ('rod', ROD, str(FUEL_ROD),   FUEL_ROD),
                                               ('ring',RING,str(FUEL_RING_ANY), FUEL_RING_ANY)]:
            fuel_label = cfg["preset"] if is_enabled else 'none'
            fh.write(f'{region}_fuel={fuel_label}  enabled={flag}\n')
        ...
        # v16: Biermann reseed configuration
        fh.write(f'\n[biermann_reseed]\n')
        fh.write(f'enabled={bool(args.biermann_reseed and IS_ROTATING)}\n')
        if args.biermann_reseed and IS_ROTATING:
            try:
                fh.write(f'drive_mode={_RESEED_DRIVE_MODE}\n')
            except NameError:
                fh.write(f'drive_mode=unavailable\n')
            fh.write(f'amplitude_T={args.biermann_reseed_amplitude_T}\n')
            fh.write(f'threshold={args.biermann_reseed_threshold}\n')
            fh.write(f'density_couple={args.biermann_reseed_density_couple}\n')
            fh.write(f'topology={getattr(args, "biermann_reseed_topology", "match-seed")}\n')
            fh.write(f'topology_resolved={get_reseed_topology()}\n')
        else:
            fh.write(f'amplitude_T=0\nthreshold=0\n'
                     f'density_couple=none\ntopology=none\n')
...
# v16 write-probe-only early exit. If the user asked for --biermann-reseed
# --biermann-reseed-write-probe-only, the reseed callback's first invocation
# (during simulation.step(1) above) has already run the probe and logged the
# result. We exit cleanly here rather than running the full simulation.
if (getattr(args, 'biermann_reseed_write_probe_only', False)
        and args.biermann_reseed and IS_ROTATING):
    if rank == 0:
        print()
        print('=' * 70)
        print('  v16 WRITE-PROBE-ONLY MODE — exiting after probe.')
        print('=' * 70)
        try:
            mode = _reseed_state['write_available']  # noqa: F821
            print(f'  Field-write probe result: {mode}')
            if mode == 'wrapper':
                print('  -> Modern pywarpx.fields wrapper API works.')
                print('  -> Reseed will fully apply on rising-edge events.')
            elif mode == 'getter':
                print('  -> Legacy getter API available (in-place modification).')
                print('  -> Reseed will attempt to apply; verify via field dumps.')
            else:
                print('  -> NO field-write access on this build.')
                print('  -> Reseed will log events but cannot inject ΔB.')
                print('  -> Required: rebuild WarpX with PYTHON_BINDINGS=ON ')
                print('     and ensure pywarpx.fields.ByWrapper is importable.')
        except Exception as e:
            print(f'  Probe-state inspection failed: {e}')
        print('=' * 70)
    sys.exit(0)
...
if rank == 0:
    print()
    print('=' * 70)
    print('  SIMULATION COMPLETE — v16.0 (Biermann reseed)')
    print('=' * 70)
